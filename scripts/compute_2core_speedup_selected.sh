...
do
    ...
    trace0=`sed -n ''$i'p' /scratch/cluster/haowu/isb-meta/ChampSim_DPC3/sim_list/2core_selected_workloads.txt | awk '{print $1}'`
    trace1=`sed -n ''$i'p' /scratch/cluster/haowu/isb-meta/ChampSim_DPC3/sim_list/2core_selected_workloads.txt | awk '{print $2}'`
   ...
   # echo "$core0_baseline_ipc $core0_sc_ipc" 
    weighted_dut=`echo "($core0_dut_ipc/$core0_sc_ipc) + ($core1_dut_ipc/$core1_sc_ipc)" | bc -l`
    weighted_baseline=`echo "($core0_baseline_ipc/$core0_sc_ipc) + ($core1_baseline_ipc/$core1_sc_ipc)" | bc -l`
